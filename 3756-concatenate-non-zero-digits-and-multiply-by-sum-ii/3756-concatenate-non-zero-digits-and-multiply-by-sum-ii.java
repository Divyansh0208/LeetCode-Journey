class Solution {
    ...
    public int[] sumAndMultiply(String s, int[][] queries) {
        int n = s.length();
        ArrayList<Integer> pos = new ArrayList<>(), digit = new ArrayList<>();
        for (int i = 0; i < n; i++) {
            int d = s.charAt(i) - '0';
            if (d != 0) {
                pos.add(i);
        ...
        long[] hash = new long[m + 1], sum = new long[m + 1];
        for (int i = 0; i < m; i++) {
        ...
        for (int i = 0; i < queries.length; i++) {
            int l = lowerBound(pos, queries[i][0]), r = upperBound(pos, queries[i][1]) - 1;
            if (l > r) {
                ans[i] = 0;
                continue;
            }
