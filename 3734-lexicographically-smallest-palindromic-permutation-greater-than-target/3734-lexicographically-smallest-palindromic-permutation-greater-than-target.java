class Solution {
    public String lexPalindromicPermutation(String s, String target) {
        ...
        if(!backtrack(half, 0, halfcnt, target, false, oddch, n)) return "";
        StringBuilder sb = new StringBuilder();
        sb.append(half);
        if(n%2 != 0) sb.append(oddch);
        for(int i = halflen-1; i >= 0; i--) sb.append(half[i]);
        return sb.toString();
    }
    private boolean backtrack(char[] half, int idx, int[] cnt, String target, boolean isGreater, char oddch, int n) {
        if(idx == half.length) {
            ...
            full.append(half);
            if(n%2 != 0) full.append(oddch);
            for(int i = half.length-1; i >= 0; i--) full.append(half[i]);
        ...
        for(int i = 0; i < 26; i++) {
            if(cnt[i] > 0) {
                char curr = (char) ('a'+i);
                if(!isGreater && curr < targetch) continue;
                cnt[i]--;
                half[idx] = curr;
