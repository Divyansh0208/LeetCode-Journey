class SparseTable {
    ...
    public SparseTable(List<Integer> data) {
        ...
        while (2 * i <= N + 1) {
            ...
            for (int j = 0; j < N - 2 * i + 1; j++) cur.add(Math.max(pre.get(j), pre.get(j + i)));
            st.add(cur);
            i <<= 1;
        }
    }
...
class Solution {
    public List<Integer> maxActiveSectionsAfterTrade(String s, int[][] queries) {
        ...
        List<Integer> zeroBlocks = new ArrayList<>(), blockLeft = new ArrayList<>(), blockRight = new ArrayList<>();
        int i = 0;
        while (i < n) {
            int st = i;
        ...
        for (int k = 0; k < m - 1; k++) gain.add(zeroBlocks.get(k) + zeroBlocks.get(k + 1));
        SparseTable sparse = new SparseTable(gain);
        List<Integer> ans = new ArrayList<>();
        for (int[] q : queries) {
            int l = q[0], r = q[1], idx = lowerBound(blockRight, l), jdx = upperBound(blockLeft, r) - 1;
            if (idx > m - 1 || jdx < 0 || idx >= jdx) {
                ans.add(cnt1);
                continue;
            }
            int firstLen = blockRight.get(idx) - Math.max(blockLeft.get(idx), l) + 1, lastLen = Math.min(blockRight.get(jdx), r) - blockLeft.get(jdx) + 1;
            if (idx + 1 == jdx) {
                ans.add(cnt1 + firstLen + lastLen);
                continue;
            }
            int val1 = firstLen + zeroBlocks.get(idx + 1), val2 = zeroBlocks.get(jdx - 1) + lastLen, val3 = sparse.query(idx + 1, jdx - 2);
