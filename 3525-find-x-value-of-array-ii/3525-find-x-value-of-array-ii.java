class Data {
    ...
    static Data getMergedData(Data d1, Data d2) {
        ...
        for (int i = 0; i < k; i++) d.pre[i] = d1.pre[i];
        for (int i = 0; i < k; i++) d.pre[(d1.mul * i) % k] += d2.pre[i];
        return d;
    }
}
class SegmentTree {
    ...
    public Data query(int l, int r) {
        if (l > r) throw new IllegalArgumentException("l is bigger than r");
        if (l < 0 || r >= len) throw new IllegalArgumentException("Invalid index values. l: " + l + " r: " + r);
        return queryHelper(0, len - 1, 0, l, r);
    }
    public void setVal(int i, int newVal) {
        if (i < 0 || i >= len) throw new IllegalArgumentException("Invalid index. i: " + i);
        if (arr[i] % Data.k == newVal % Data.k) return;
        arr[i] = newVal;
    ...
    private void build(int i, int left, int right, int[] arr) {
        if (left == right) {
            ...
            for (int j = 0; j < Data.k; j++) tree[i].pre[j] = arr[left] % Data.k == j ? 1 : 0;
            return;
        }
    ...
    private void updateHelper(int l, int r, int i, int newVal, int indexToUpdate) {
        if (l == r) {
            for (int j = 0; j < Data.k; j++) tree[i].pre[j] = newVal % Data.k == j ? 1 : 0;
            tree[i].mul = newVal % Data.k;
            return;
        }
        int mid = (l + r) / 2;
        if (indexToUpdate <= mid) updateHelper(l, mid, 2 * i + 1, newVal, indexToUpdate);
        else updateHelper(mid + 1, r, 2 * i + 2, newVal, indexToUpdate);
        tree[i] = Data.getMergedData(tree[2 * i + 1], tree[2 * i + 2]);
    }
}
class Solution {
